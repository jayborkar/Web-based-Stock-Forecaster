...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 21 23:37:02 2018
...
def openfile(i):
    csv = numpy.genfromtxt ('test.csv', delimiter=",")
    data = csv[i]
	#print (data)
    return data

# function to find the performance
def performance(meanvalue, average,ame):	
    print ('')
    print ('Performance - ')
     
    
    print('Absolute mean error :', ame)# absolute mean error
...
def avg(targetdata, meanvalue):
    ...
    for a in targetdata:
        sum += a 
        abs=abs + numpy.absolute(a-meanvalue)
    
    average = sum / len(targetdata)
    ame=abs/len(targetdata) # calculating the absolute mean error
    
    print ('Actual average :', average)
    print ('Calculated prediction average : ', meanvalue)
    performance(meanvalue, average,ame)
